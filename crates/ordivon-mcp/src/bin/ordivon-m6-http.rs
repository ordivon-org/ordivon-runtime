...
use axum::Router;
use ordivon_exec::{M6RegistryConfig, M6Runtime, M6RuntimeConfig, UniversalExecutorConfig};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !app.bind.ip().is_loopback() {
        return Err("ORDIVON_M6_BIND must use a loopback address".into());
    }
...
fn load_config() -> Result<AppConfig, Box<dyn std::error::Error>> {
    let bind: SocketAddr = std::env::var("ORDIVON_M6_BIND")
        .unwrap_or_else(|_| "127.0.0.1:8896".to_string())
        .parse()?;
    let token = required_env("ORDIVON_M6_BEARER_TOKEN")?;
    if token.len() < 32 {
        return Err("ORDIVON_M6_BEARER_TOKEN must be at least 32 characters".into());
    }
    let store_root = PathBuf::from(required_env("ORDIVON_M6_STORE_ROOT")?);
    let registry_root = PathBuf::from(required_env("ORDIVON_M6_REGISTRY_ROOT")?);
    let runner_path = PathBuf::from(required_env("ORDIVON_M6_RUNNER_PATH")?);
    let roots = required_env("ORDIVON_M6_ALLOWED_EXECUTABLE_ROOTS")?;
    let allowed_executable_roots = roots
        .split(':')
        .filter(|value| !value.is_empty())
        .map(PathBuf::from)
        .collect();
    let trace_path = std::env::var("ORDIVON_M6_TRACE_PATH")
        .ok()
        .map(PathBuf::from)
        .or_else(|| Some(registry_root.join("m6-trace.jsonl")));
    let allowed_origins = std::env::var("ORDIVON_M6_ALLOWED_ORIGINS")
        .unwrap_or_else(|_| "http://localhost,http://127.0.0.1".to_string())
        .split(',')
        .filter(|value| !value.is_empty())
        .map(ToString::to_string)
        .collect();
    let body_limit_bytes = std::env::var("ORDIVON_M6_BODY_LIMIT_BYTES")
        .ok()
        .map(|value| value.parse())
        .transpose()?
        .unwrap_or(1_048_576);
    let busy_timeout_ms = std::env::var("ORDIVON_M6_BUSY_TIMEOUT_MS")
        .ok()
        .map(|value| value.parse())
        .transpose()?
        .unwrap_or(5_000);
    let startup_grace_ms = std::env::var("ORDIVON_M6_STARTUP_GRACE_MS")
        .ok()
        .map(|value| value.parse())
        .transpose()?
        .unwrap_or(2_000);
...
fn load_dogfood_policy() -> Result<Option<M5DogfoodPolicy>, Box<dyn std::error::Error>> {
    let repos = std::env::var("ORDIVON_M6_ALLOWED_SOURCE_REPOS").ok();
    let revisions = std::env::var("ORDIVON_M6_ALLOWED_SOURCE_REVISIONS").ok();
    match (repos, revisions) {
        ...
        _ => Err(
            "ORDIVON_M6_ALLOWED_SOURCE_REPOS and ORDIVON_M6_ALLOWED_SOURCE_REVISIONS must be set together"
                .into(),
        ),
    }
}
