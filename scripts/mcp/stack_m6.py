#!/usr/bin/env python3
"""Reproducible local lifecycle controller for Ordivon M6 transactional dogfood."""
...
DEFAULT_STATE_ROOT = Path("/root/.local/share/ordivon-m6-stack")
...
def build_binaries() -> tuple[Path, Path]:
    ...
    server = (REPO_ROOT / "target/debug/ordivon-m6-http").resolve()
    runner = (REPO_ROOT / "target/debug/ordivon-task-runner").resolve()
    if not server.is_file() or not runner.is_file():
        raise RuntimeError("M6 binaries were not produced")
...
def load_manifest(state_root: Path) -> dict[str, Any]:
    ...
    if not path.is_file():
        raise RuntimeError(f"no active M6 stack manifest at {path}")
...
def stop_stack(state_root: Path, *, tolerate_missing: bool = False) -> dict[str, Any]:
    ...
    if not manifest_path.is_file():
        ...
        raise RuntimeError(f"no active M6 stack manifest at {manifest_path}")
...
def start_stack(state_root: Path, port: int | None) -> dict[str, Any]:
    ...
    unit = f"ordivon-m6-http-{run_id}.service"
    store_root = (state_root / "store").resolve()
    registry_root = (state_root / "registry").resolve()
    token = secrets.token_hex(32)
    environment_file = (state_root / "server.env").resolve()
    trace_path = (registry_root / "m6-core-trace.jsonl").resolve()
    state_root.mkdir(parents=True, exist_ok=True)
    state_root.chmod(0o700)
    environment_file.write_text(
        "\n".join(
            [
                f"ORDIVON_M6_BIND=127.0.0.1:{selected_port}",
                f"ORDIVON_M6_STORE_ROOT={store_root}",
                f"ORDIVON_M6_REGISTRY_ROOT={registry_root}",
                f"ORDIVON_M6_BEARER_TOKEN={token}",
                f"ORDIVON_M6_RUNNER_PATH={runner}",
                f"ORDIVON_M6_ALLOWED_EXECUTABLE_ROOTS=/usr/bin:{toolchain_root}",
                f"ORDIVON_M6_TRACE_PATH={trace_path}",
                f"ORDIVON_M6_ALLOWED_SOURCE_REPOS={REPO_ROOT.resolve()}",
                f"ORDIVON_M6_ALLOWED_SOURCE_REVISIONS={git_head()}",
                "ORDIVON_M6_BUSY_TIMEOUT_MS=5000",
                "ORDIVON_M6_STARTUP_GRACE_MS=2000",
            ]
        )
        + "\n"
    )
...
def child_environment(manifest: dict[str, Any]) -> dict[str, str]:
    ...
    environment.update(
        {
            "ORDIVON_M6_MCP_URL": manifest["endpoint"],
            "ORDIVON_M6_BEARER_TOKEN": manifest["token"],
            "ORDIVON_MCP_SDK_ROOT": manifest["sdkRoot"],
            "ORDIVON_M6_STORE_ROOT": manifest["storeRoot"],
            "ORDIVON_M6_REGISTRY_ROOT": manifest["registryRoot"],
            "ORDIVON_M6_REGISTRY_DB": manifest["registryDb"],
            "ORDIVON_M6_REPO_ROOT": manifest["repoRoot"],
            "ORDIVON_M6_SOURCE_REVISION": manifest["sourceRevision"],
            "ORDIVON_M6_TRACE_PATH": manifest["tracePath"],
            "ORDIVON_M6_HTTP_TRACE_PATH": manifest["httpTracePath"],
            "ORDIVON_M6_CARGO_BINARY": manifest["cargoBinary"],
            "ORDIVON_M6_RUSTC_BINARY": manifest["rustcBinary"],
        }
    )
    return environment
