...
const workspaceId = `m7-wire-workspace-${suffix}`;
const clientRequestId = `m7-wire-request-${suffix}`;
const connection = await connectM7('ordivon-m7-wire-contract');
const observations = [];
...
function execution(script, waitMs = 10_000) {
  return {
    schemaVersion: 1,
    clientRequestId,
    principal: 'principal:m7-wire',
    authorityRef: 'authority:m7-local-dogfood',
    policyId: 'policy:m7-local-dogfood',
    policyVersion: '1',
    policyDigest: sha256('policy:m7-local-dogfood:1'),
    globalLimit: 4,
    execution: {
      workspaceId,
...
try {
  ...
  await measured('workspace.mutate', {
    ...
    workspaceId,
    mutations: [{
      relativePath: 'm7_wire.py',
      mode: 'WRITE',
      content: "print('M7_WIRE_OK', flush=True)\n",
      expectedDigest: null
    }]
  }, 480);

  const first = await measured('workspace.exec', execution('m7_wire.py'), 640);
  assert.equal(first.status, 'succeeded');
  assert.match(first.stdoutTail, /M7_WIRE_OK/);
  assert.match(first.jobId, /^job-/);
  assert.match(first.attemptId, /^attempt-/);

  const replay = await measured('workspace.exec', execution('m7_wire.py'), 640);
  ...
  assert.match(artifact.content, /M7_WIRE_OK/);
  ...
  assert.ok(diff.untrackedPaths.includes('m7_wire.py'));
  ...
  await measured('workspace.mutate', {
    ...
    mutations: [
      {
        relativePath: 'm7_native.py',
        mode: 'WRITE',
        content: "import time\nprint('M7_NATIVE_START', flush=True)\ntime.sleep(1.0)\nprint('M7_NATIVE_DONE', flush=True)\n",
        expectedDigest: null
      },
      {
  ...
  const nativeJobId = await startNative(
    connection,
    'm7_native.py',
    `m7-native-request-${suffix}`
  );
  assert.match(nativeJobId, /^job-/);
  await closeConnection(connection);
  await new Promise(resolve => setTimeout(resolve, 1300));

  const resumed = await connectM7('ordivon-m7-wire-resumed');
  try {
    ...
    assert.match(nativeResult.structuredContent.stdoutTail, /M7_NATIVE_DONE/);

    const cancelJobId = await startNative(
      resumed,
      'm7_cancel.py',
      `m7-cancel-request-${suffix}`
    );
    ...
    console.log(JSON.stringify({
      schemaVersion: 1,
      phase: 'ORDIVON-M7-WIRE-CONTRACT',
      sourceRevision: config.sourceRevision,
      workspaceId,
