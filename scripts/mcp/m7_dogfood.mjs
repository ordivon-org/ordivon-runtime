...
const outputPath = resolve(
  outputIndex >= 0 ? process.argv[outputIndex + 1] : '/tmp/ordivon-m7-dogfood.json'
);
const runSuffix = `${process.pid}-${Date.now()}`;
const policyDigest = sha256('policy:m7-limited-dogfood:1');
...
function execRequest(workspaceId, clientRequestId, executable, args, env = {}, waitMs = 30_000) {
  return {
    schemaVersion: 1,
    clientRequestId,
    principal: 'principal:m7-dogfood',
    authorityRef: 'authority:m7-local-dogfood',
    policyId: 'policy:m7-limited-dogfood',
    policyVersion: '1',
    policyDigest,
    globalLimit: 8,
    execution: {
...
async function openWorkspace(connection, state, label) {
  const workspaceId = `m7-${label}-${runSuffix}`;
...
async function runJourney(name, operation) {
  const connection = await connectM7(`ordivon-m7-${name}`);
  const state = metrics();
...
journeys.push(await runJourney('readonly-audit', async (connection, state) => {
  ...
  const audit = (await measured(connection, state, 'workspace.exec', execRequest(
    workspaceId,
    `m7-readonly-${runSuffix}`,
    '/usr/bin/rg',
    ['-n', 'runtime-hardening-m7', 'crates/ordivon-exec/Cargo.toml']
  ))).value;
  assert.equal(audit.status, 'succeeded');
  assert.match(audit.stdoutTail, /runtime-hardening-m7/);
...
journeys.push(await runJourney('single-file-edit', async (connection, state) => {
  ...
  const marker = `<!-- M7_SINGLE_${runSuffix} -->`;
...
journeys.push(await runJourney('multi-file-test', async (connection, state) => {
  ...
  await measured(connection, state, 'workspace.mutate', {
    ...
    mutations: [
      {
        relativePath: 'm7calc.py',
        mode: 'WRITE',
        content: 'def multiply(left, right):\n    return left * right\n',
        expectedDigest: null
      },
      {
        relativePath: 'test_m7calc.py',
        mode: 'WRITE',
        content: "import unittest\nfrom m7calc import multiply\n\nclass TestCalc(unittest.TestCase):\n    def test_multiply(self):\n        self.assertEqual(multiply(6, 7), 42)\n\nif __name__ == '__main__':\n    unittest.main()\n",
        expectedDigest: null
      }
    ]
  });
  ...
  assert.ok(diff.untrackedPaths.includes('m7calc.py'));
  assert.ok(diff.untrackedPaths.includes('test_m7calc.py'));
  return {
    workspaceId,
    jobId: test.jobId,
...
journeys.push(await runJourney('failure-repair-loop', async (connection, state) => {
  ...
  await measured(connection, state, 'workspace.mutate', {
    ...
    mutations: [
      {
        relativePath: 'm7_bug.py',
        mode: 'WRITE',
        content: 'def add(left, right):\n    return left - right\n',
        expectedDigest: null
      },
      {
        relativePath: 'test_m7_bug.py',
        mode: 'WRITE',
        content: "import unittest\nfrom m7_bug import add\n\nclass TestBug(unittest.TestCase):\n    def test_add(self):\n        self.assertEqual(add(2, 3), 5)\n\nif __name__ == '__main__':\n    unittest.main()\n",
        expectedDigest: null
      }
    ]
  });
  const failed = (await measured(connection, state, 'workspace.exec', execRequest(
    workspaceId,
    `m7-repair-fail-${runSuffix}`,
    '/usr/bin/python3.14',
    ['-m', 'unittest', '-v', 'test_m7_bug.py'],
    { PYTHONDONTWRITEBYTECODE: '1' }
  ))).value;
...
journeys.push(await runJourney('python-package-validation', async (connection, state) => {
  const workspaceId = await openWorkspace(connection, state, 'package');
  await measured(connection, state, 'workspace.mutate', {
    schemaVersion: 1,
    workspaceId,
    mutations: [
      {
        relativePath: 'm7pkg/__init__.py',
        mode: 'WRITE',
        content: "from .ledger import Ledger\n",
        expectedDigest: null
      },
      {
        relativePath: 'm7pkg/ledger.py',
        mode: 'WRITE',
        content: "class Ledger:\n    def __init__(self):\n        self._values = []\n    def append(self, value):\n        if value < 0:\n            raise ValueError('negative')\n        self._values.append(value)\n    def total(self):\n        return sum(self._values)\n",
        expectedDigest: null
      },
      {
        relativePath: 'test_m7pkg.py',
        mode: 'WRITE',
        content: "import unittest\nfrom m7pkg import Ledger\n\nclass LedgerTest(unittest.TestCase):\n    def test_many_values(self):\n        ledger = Ledger()\n        for value in range(5000):\n            ledger.append(value)\n        self.assertEqual(ledger.total(), sum(range(5000)))\n    def test_negative_rejected(self):\n        with self.assertRaises(ValueError):\n            Ledger().append(-1)\n\nif __name__ == '__main__':\n    unittest.main()\n",
        expectedDigest: null
      }
    ]
  });
  const validation = (await measured(connection, state, 'workspace.exec', execRequest(
    workspaceId,
    `m7-package-${runSuffix}`,
    '/usr/bin/python3.14',
    ['-m', 'unittest', '-v', 'test_m7pkg.py'],
    { PYTHONDONTWRITEBYTECODE: '1' }
  ))).value;
  assert.equal(validation.status, 'succeeded');
  assert.match(validation.stderrTail, /Ran 2 tests/);
  assert.match(validation.stderrTail, /OK/);
  state.outputBytes += byteLength(validation.stdoutTail) + byteLength(validation.stderrTail);
  return {
    workspaceId,
    jobId: validation.jobId,
    semanticDigest: sha256(`${validation.status}:m7-python-package-validation`),
    repairRounds: 0,
    workerPackageValidation: true
  };
}));
...
journeys.push(await runJourney('bounded-log-artifact', async (connection, state) => {
  ...
  await measured(connection, state, 'workspace.mutate', {
    ...
    workspaceId,
    mutations: [{
      relativePath: 'm7_log.py',
      mode: 'WRITE',
      content: "for i in range(20000):\n    print(f'M7_LOG_{i:05d}_' + 'x' * 32)\n",
      expectedDigest: null
    }]
  });
  ...
  assert.match(artifact.content, /M7_LOG_00000/);
...
journeys.push(await runJourney('durable-recovery-cancel', async (connection, state) => {
  ...
  await measured(connection, state, 'workspace.mutate', {
    ...
    mutations: [
      {
        relativePath: 'm7_recover.py',
        mode: 'WRITE',
        content: "import time\nprint('M7_RECOVER_START', flush=True)\ntime.sleep(1.2)\nprint('M7_RECOVER_DONE', flush=True)\n",
        expectedDigest: null
      },
      {
        relativePath: 'm7_cancel.py',
        mode: 'WRITE',
        content: "import time\nprint('M7_CANCEL_START', flush=True)\ntime.sleep(30)\n",
        expectedDigest: null
      }
    ]
  });
  const recoverJobId = await startNative(
    ...
    workspaceId,
    'm7_recover.py',
    `m7-recover-${runSuffix}`
  );
  await closeConnection(connection);
  await new Promise(resolve => setTimeout(resolve, 1500));

  const resumed = await connectM7('ordivon-m7-durable-resumed');
  let cancelJobId;
  try {
    ...
    assert.match(result.structuredContent.stdoutTail, /M7_RECOVER_DONE/);
...
journeys.push(await runJourney('idempotency-and-list', async (connection, state) => {
  ...
  await measured(connection, state, 'workspace.mutate', {
    ...
    workspaceId,
    mutations: [{
      relativePath: 'm7_idempotent.py',
      mode: 'WRITE',
      content: "print('M7_IDEMPOTENT_OK', flush=True)\n",
      expectedDigest: null
    }]
  });
  const requestId = `m7-idempotency-${runSuffix}`;
  const request = execRequest(
    workspaceId,
    requestId,
    '/usr/bin/python3.14',
  ...
  assert.match(first.stdoutTail, /M7_IDEMPOTENT_OK/);
...
const evidence = {
  schemaVersion: 1,
  phase: 'ORDIVON-MIGRATION-M7-LIMITED-DOGFOOD-2026-07-22',
  generatedAt: new Date().toISOString(),
  sourceRevision: config.sourceRevision,
  routePolicy: {
    primaryBackend: 'ORDIVON_M7_HARDENED_WORKER_MCP',
    sourceRepo: config.repoRoot,
    sourceRevision: config.sourceRevision,
    ...
    allowedCapabilities: [
      ...
      'sandboxed transactional Job execution under ordivon-worker',
  ...
  claimsNotMade: [
    ...
    'M7 Worker isolation is local-only and does not establish remote or production authority.'
  ]
};
