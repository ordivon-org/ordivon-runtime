#!/usr/bin/env python3
"""Run one child command with independent M6 and M7 local MCP stacks."""
...
M6_STATE = Path("/root/.local/share/ordivon-m6-m7-shadow/m6")
M7_STATE = Path("/root/.local/share/ordivon-m6-m7-shadow/m7")
...
def main() -> int:
    ...
    m7 = load_module("ordivon_stack_m7", Path(__file__).with_name("stack_m7.py"))
    m6_manifest = None
    m7_manifest = None
    try:
        m6_manifest = m6.start_stack(M6_STATE, None)
        m7_manifest = m7.start_stack(M7_STATE, None)
        environment = os.environ.copy()
        environment.update(m6.child_environment(m6_manifest))
        environment.update(m7.child_environment(m7_manifest))
        return subprocess.run(child, cwd=REPO_ROOT, env=environment, check=False).returncode
    finally:
        if m7_manifest is not None:
            m7.stop_stack(M7_STATE, tolerate_missing=True)
...
if __name__ == "__main__":
    try:
        ...
        print(f"M6_M7_STACK_ERROR: {error}", file=sys.stderr)
        raise SystemExit(1)
