...
const endpoint = new URL(args.get('--mcp-url') ?? process.env.ORDIVON_M6_MCP_URL ?? 'http://127.0.0.1:8896/mcp');
const outputPath = resolve(args.get('--output') ?? '/tmp/ordivon-m6-security.json');
const token = process.env.ORDIVON_M6_BEARER_TOKEN ?? '';
const sourceRevision = process.env.ORDIVON_M6_SOURCE_REVISION ?? '';
if (token.length < 32) throw new Error('ORDIVON_M6_BEARER_TOKEN must be at least 32 characters');
...
const initialize = JSON.stringify({
  ...
  params: {
    ...
    capabilities: {},
    clientInfo: { name: 'm6-security', version: '0.1.0' }
  }
});
...
const evidence = {
  schemaVersion: 1,
  phase: 'ORDIVON-MIGRATION-M6-TRANSPORT-SECURITY-2026-07-22',
