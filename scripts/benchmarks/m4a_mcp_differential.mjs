...
const config = {
  ...
  outputPath: resolve(args.get('--output') ?? '/tmp/ordivon-m4a-evidence.json'),
...
async function runLegacyJourney(pairId, marker) {
  ...
  return {
    ...
    remoteRoundTrips: transportDelta(transportSnapshot(bootstrap), transportAtTaskStart).httpRequests,
...
async function runOrdivonJourney(pairId, marker) {
  const metrics = makeMetrics();
  const bootstrap = makeMetrics();
  const workspaceId = `m4a-workspace-${pairId}`;
  const taskId = `m4a-task-${pairId}`;
  const workspacePath = join(config.m4StoreRoot, 'workspaces', workspaceId);
  const material = journeyMaterial(marker);
  removeWorktree(workspacePath);
  const connection = await connectM4(bootstrap, `ordivon-m4a-${pairId}`);
  const transportAtTaskStart = transportSnapshot(bootstrap);
  const started = process.hrtime.bigint();
  let succeeded = false;
  let semanticDigest = null;
  ...
  try {
    ...
    if (compact.status !== 'COMPLETED') throw new Error(`M4A task failed: ${JSON.stringify(compact)}`);
    if (!compact.stdoutTail.includes(material.expectedStdout)) throw new Error('M4A stdout mismatch');
    if (!compact.stderrTail.includes(material.expectedStderr)) throw new Error('M4A stderr mismatch');
    metrics.outputBytes += byteLength(compact.stdoutTail) + byteLength(compact.stderrTail);
    const generated = await m4Call(connection, metrics, 'workspace.read', {
      schemaVersion: 1,
      workspaceId,
    ...
    if (generated.content !== material.expectedOutput) {
      throw new Error('M4A generated output mismatch');
    }
    ...
    if (!diff.diff.includes(material.appendedLine)) throw new Error('M4A diff lost marker');
    if (!diff.untrackedPaths.includes('m2_tool.py') || !diff.untrackedPaths.includes('m2-output.txt')) {
      throw new Error('M4A untracked paths incomplete');
    }
...
for (let index = 0; index < totalPairs; index += 1) {
  ...
  console.error(
    `M4A_PAIR index=${index} warmup=${index < config.warmups} ` +
...
const evidence = {
  schemaVersion: SCHEMA_VERSION,
  phase: 'ORDIVON-MIGRATION-M4A-2026-07-22',
  evidenceClass: 'MCP_TO_MCP_M4A_DIFFERENTIAL_BENCHMARK',
  generatedAt: new Date().toISOString(),
  sourceRevision: config.sourceRevision,
  taskJourney: 'M4A_FULL_READ_MCP_TASK_JOURNEY',
  metricSemantics: {
