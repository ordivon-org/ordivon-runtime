...
const config = {
  ...
  sourceRevision: args.get('--source-revision') ?? '59e59254316e7cdcb52e8c824f8c7955f8808e8f',
  ...
  outputPath: resolve(args.get('--output') ?? '/tmp/ordivon-m2-evidence.json'),
...
function runOrdivonJourney(pairId, marker) {
  const metrics = makeMetrics();
  const storeRoot = join('/root/.local/share/ordivon-m2', `m1-store-${pairId}`);
  const workspaceId = `m2-workspace-${pairId}`;
  const taskId = `m2-task-${pairId}`;
  ...
  let elapsedMs = 0;
  try {
    m1Call(metrics, storeRoot, 'workspace-create', {
      schemaVersion: 1,
      workspaceId,
      sourceRepo: config.repoRoot,
      sourceRevision: config.sourceRevision
    });
    const read = m1Call(metrics, storeRoot, 'workspace-read', {
      schemaVersion: 1,
      workspaceId,
      relativePath: 'crates/ordivon-exec/README.md',
      maxBytes: 1_048_576
    });
    const updated = `${read.content}\n${material.appendedLine}\n`;
    m1Call(metrics, storeRoot, 'workspace-write', {
      schemaVersion: 1,
      workspaceId,
      relativePath: 'crates/ordivon-exec/README.md',
      content: updated,
    ...
    const startedTask = m1Call(metrics, storeRoot, 'task-start', {
      schemaVersion: 1,
      taskId,
      workspaceId,
    ...
    if (startedTask.status !== 'WORKING') {
      throw new Error(`M1 task did not start: ${JSON.stringify(startedTask)}`);
    }
    const completed = m1Call(metrics, storeRoot, 'task-get', {
      schemaVersion: 1,
      taskId,
      waitMs: 5_000
    });
    if (completed.status !== 'COMPLETED') {
      throw new Error(`M1 task did not complete: ${JSON.stringify(completed)}`);
    }
    const stdout = m1Call(metrics, storeRoot, 'artifact-read', {
      schemaVersion: 1,
      taskId,
    ...
    const stderr = m1Call(metrics, storeRoot, 'artifact-read', {
      schemaVersion: 1,
      taskId,
      artifactId: `${taskId}.stderr`,
      offset: 0,
      maxBytes: 65_536
    });
    if (!stdout.content.includes(material.expectedStdout)) {
      throw new Error(`M1 stdout mismatch: ${stdout.content}`);
    }
    if (!stderr.content.includes(material.expectedStderr)) {
      throw new Error(`M1 stderr mismatch: ${stderr.content}`);
    }
    metrics.outputBytes += byteLength(stdout.content) + byteLength(stderr.content);
    const generated = m1Call(metrics, storeRoot, 'workspace-read', {
      schemaVersion: 1,
      workspaceId,
      relativePath: 'm2-output.txt',
      maxBytes: 1_024
    });
    if (generated.content !== material.expectedOutput) {
      throw new Error(`M1 generated output mismatch: ${generated.content}`);
    }
    const diff = m1Call(metrics, storeRoot, 'workspace-diff', {
      schemaVersion: 1,
      workspaceId,
      maxBytes: 1_048_576
    });
    if (!diff.diff.includes(material.appendedLine)) throw new Error('M1 diff lost marker');
    if (!diff.untrackedPaths.includes('m2_tool.py') || !diff.untrackedPaths.includes('m2-output.txt')) {
      throw new Error(`M1 untracked paths incomplete: ${JSON.stringify(diff.untrackedPaths)}`);
    }
    semanticDigest = sha256(
      JSON.stringify({
...
function assessCutover(legacy, ordivon) {
  const gates = {
    ...
    toolCallsReduced30Percent:
      reductionPercent(legacy.toolCalls, ordivon.toolCalls) >= 30,
    contextReduced50Percent:
      reductionPercent(legacy.contextBytes, ordivon.contextBytes) >= 50,
    elapsedWithin10Percent:
      ordivon.elapsedMs <= Math.round(legacy.elapsedMs * 1.1),
    disconnectRecovery: ordivon.recoveredAfterDisconnect,
    noFallback: ordivon.fallbackCount === 0,
    semanticEquivalence:
      JSON.stringify(legacy.semanticDigests) ===
      JSON.stringify(ordivon.semanticDigests)
  };
  return {
    ...
    elapsedRatio: legacy.elapsedMs === 0 ? null : ordivon.elapsedMs / legacy.elapsedMs
  };
}
...
for (let index = 0; index < totalPairs; index += 1) {
  ...
  console.error(
    `M2_PAIR index=${index} warmup=${index < config.warmups} ` +
...
const evidence = {
  ...
  evidenceClass: 'LOCAL_DIFFERENTIAL_BENCHMARK',
  generatedAt: new Date().toISOString(),
  sourceRevision: config.sourceRevision,
  taskJourney: 'MODEL_AUTHORED_PYTHON_TOOL_IN_ISOLATED_GIT_WORKTREE',
  metricSemantics: {
    ...
    ordivonTransport: 'M1 is local CLI; no MCP transport result is fabricated.'
  },
  runtimeIdentity: {
  ...
  cutover,
  claimsNotMade: [
    'Ordivon M1 has no MCP transport, so no Ordivon HTTP byte claim is made.',
