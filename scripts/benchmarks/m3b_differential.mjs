...
const config = {
  ...
  outputPath: resolve(args.get('--output') ?? '/tmp/ordivon-m3b-evidence.json'),
...
async function runLegacyJourney(pairId, marker) {
  ...
  return {
    ...
    toolCalls: metrics.logicalCalls, remoteRoundTrips: metrics.logicalCalls,
...
function runOrdivonJourney(pairId, marker) {
  const metrics = makeMetrics();
  const storeRoot = join('/root/.local/share/ordivon-m3', `m3b-store-${pairId}`);
  const workspaceId = `m3b-workspace-${pairId}`;
  const taskId = `m3b-task-${pairId}`;
  ...
  try {
    m1Call(metrics, storeRoot, 'workspace-open', {
      ...
      sourceRepo: config.repoRoot, sourceRevision: config.sourceRevision
    });
    const read = m1Call(metrics, storeRoot, 'workspace-read-slice-compact', {
      schemaVersion: 1, workspaceId,
      relativePath: 'crates/ordivon-exec/README.md', offset: 0, maxBytes: 64
    });
    if (byteLength(read.content) > 64) throw new Error('M3B slice exceeded 64 bytes');
    m1Call(metrics, storeRoot, 'workspace-mutate', {
      schemaVersion: 1, workspaceId,
      mutations: [
        {
          relativePath: 'crates/ordivon-exec/README.md', mode: 'APPEND',
          content: `\n${material.appendedLine}\n`, expectedDigest: read.fileDigest
        },
        {
          relativePath: 'm3b_tool.py', mode: 'WRITE',
          content: material.script, expectedDigest: null
        }
      ]
    });
    const compact = m1Call(metrics, storeRoot, 'task-run', {
      schemaVersion: 1,
      execution: {
        schemaVersion: 1, taskId, workspaceId,
        executable: '/usr/bin/python3.14', args: ['m3b_tool.py'], cwdRelative: '.',
        env: { PYTHONUNBUFFERED: '1' }, timeoutMs: 10_000,
        stdoutLimitBytes: 65_536, stderrLimitBytes: 65_536
      },
      waitMs: 5000, stdoutTailBytes: 1024, stderrTailBytes: 1024
    });
    if (compact.status !== 'COMPLETED') throw new Error(`M3B task failed: ${JSON.stringify(compact)}`);
    if (!compact.stdoutTail.includes(material.expectedStdout)) throw new Error('M3B stdout mismatch');
    if (!compact.stderrTail.includes(material.expectedStderr)) throw new Error('M3B stderr mismatch');
    ...
    const diff = m1Call(metrics, storeRoot, 'workspace-diff-compact', {
      schemaVersion: 1, workspaceId, maxBytes: 1_048_576
    });
    if (!diff.diff.includes(material.appendedLine)) throw new Error('M3B diff lost marker');
    if (!diff.untrackedPaths.includes('m3b_tool.py') || !diff.untrackedPaths.includes('m3b-output.txt')) {
      throw new Error('M3B untracked paths incomplete');
    }
    semanticDigest = sha256(JSON.stringify({
      output: material.expectedOutput,
      stdout: material.expectedStdout,
...
for (let index = 0; index < totalPairs; index += 1) {
  ...
  console.error(
    `M3B_PAIR index=${index} warmup=${index < config.warmups} ` +
...
const evidence = {
  ...
  evidenceClass: 'LOCAL_M3B_DIFFERENTIAL_BENCHMARK',
  generatedAt: new Date().toISOString(),
  sourceRevision: config.sourceRevision,
  taskJourney: 'M3B_TARGETED_READ_COMPACT_TASK_JOURNEY',
  metricSemantics: {
