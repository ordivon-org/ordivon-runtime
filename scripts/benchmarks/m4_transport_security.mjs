...
const endpoint = new URL(args.get('--mcp-url') ?? 'http://127.0.0.1:8895/mcp');
const outputPath = resolve(args.get('--output') ?? '/tmp/ordivon-m4-security.json');
const token = process.env.ORDIVON_M4_BEARER_TOKEN ?? '';
if (token.length < 32) throw new Error('ORDIVON_M4_BEARER_TOKEN must be at least 32 characters');
...
const initialize = JSON.stringify({
  ...
  params: {
    ...
    capabilities: {},
    clientInfo: { name: 'm4-security', version: '0.1.0' }
  }
});
...
const evidence = {
  schemaVersion: 1,
  phase: 'ORDIVON-MIGRATION-M4-TRANSPORT-SECURITY-2026-07-22',
