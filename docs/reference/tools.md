...
| `workspace.diff` | Return Git diff text bounded by maxBytes plus the complete structured changed, modified, added, deleted, renamed, and untracked path projection for an isolated Workspace. maxBytes bounds only diff text; it does not bound the structured path-list payload. |
