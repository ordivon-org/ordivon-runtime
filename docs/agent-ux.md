...
- `task.observe` is an exact-Job convergence action rather than a passive getter; it may reconcile or dispatch already-committed work. `task.list` remains pure projection, while an exact non-reconciling `task.get(jobId)` capability is currently absent and should not be simulated by pretending `task.observe` is read-only.
