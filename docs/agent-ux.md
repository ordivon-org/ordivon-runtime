...
- immutable input bindings are a separate Core physical contract. A Runtime instance owns named `InputAuthority` roots; an action may reference only an authority name, a normal relative object path, an expected SHA-256 digest, and a presentation-relative path. New admission resolves the object beneath the configured authority with fail-closed path rules, copies and verifies the exact bytes into a Runtime-owned input set, freezes `inputSetId` plus `effectiveInputs` in the Execution Plan, and presents that set read-only to `contained_local` at `/run/ordivon/inputs`. Durable plan/evidence does not contain the authority host root or the materialization host path. Exact replay finds the existing Job before consulting the current authority or source bytes. This contract is Core-only today; the public MCP execution tools do not yet expose input-binding arguments.
