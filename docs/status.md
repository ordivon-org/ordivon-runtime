...
| explicit Host Dependency commitments | implemented for trusted-local Linux; P2 bound Agent-declared path+SHA-256 prerequisites into operation-v6, and P3 physically proved pre-spawn checks alone were insufficient for delayed runtime consumption. Runner now witnesses relevant path/topology drift from before final validation through the Attempt and fails `HOST_DEPENDENCY_RUNTIME_DRIFT`; terminal evidence distinguishes observed continuity from detected drift without claiming immutability or complete environment closure |
