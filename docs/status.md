...
| explicit Host Dependency commitments | implemented for trusted-local Linux; P2 physically proved ambient shared-library drift could change output under the same prior operation identity, then added Agent-declared path+SHA-256 prerequisite binding, operation-v6 side truth, admission/pre-dispatch/Runner checks, and terminal evidence without claiming complete environment closure |
...
- execution-provider commitment covers Runtime's own Linux Runner or Windows launcher. Explicit Host Dependencies may bind known host prerequisite files on trusted-local Linux, but Runtime still does not claim automatic shared-library/toolchain closure, kernel semantics, network responses, or an arbitrary target's external provider contract.
