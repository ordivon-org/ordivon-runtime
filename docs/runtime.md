...
Workspace reads distinguish response bounds from proof cost. FULL reads hard-cap bytes actually consumed. SLICE reads keep memory proportional to the requested slice plus a fixed streaming buffer while still validating and hashing the whole file in one pass; therefore SLICE I/O remains O(file size) because the public result still includes a whole-file digest. `workspace.diff.maxBytes` is narrower: it bounds only retained diff text. The structured changed/modified/added/deleted/renamed/untracked path arrays remain complete and may exceed that byte count, and the current Git diff implementation may still generate more work than the retained text budget. A future bounded change-list/continuation contract is required before those facts can be safely truncated.
