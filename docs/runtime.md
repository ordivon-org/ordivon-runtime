...
There is intentionally no claim that `task.observe` is a pure read: it may advance reconciliation for the targeted durable Job. `task.list` remains the non-reconciling Job projection, but it does not yet offer an exact `jobId` lookup. A future exact pure-read Job surface should be added only with a clean query contract rather than by multiplying the existing filter/cursor SQL branches.
