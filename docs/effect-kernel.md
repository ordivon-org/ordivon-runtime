...
`workspace.execBound` exposes the reduced-authority immutable-input path for bytes outside the Git Workspace without transferring host-path authority to callers. Operator-owned named authorities are resolved only on new admission; Runtime opens only normal relative regular files beneath the authority, materializes and verifies the expected SHA-256 bytes into a Job-owned set, freezes the effective bindings into the plan, re-verifies that set before dispatch, and exposes it read-only. On local Linux the ordinary Tool selects `contained_local`. `workspace.execBoundTrusted` uses the same immutable-input machinery under a distinct public operation identity while selecting `trusted_local`, so exact bytes can be consumed by owner-trusted open-world adapters without exposing the authority source path. Exact replay resolves the existing Job before current authority or policy. Neither Tool turns input presence into provider/domain authority, ambient external state into Runtime truth, or target effects into an idempotent/reconcilable Effect.
