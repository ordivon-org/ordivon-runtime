...
| Additive `inputSetId` / `effectiveInputs` Execution Plan fields and input-bound request identity | Existing Jobs whose plans predate immutable input binding | Old plans fail to decode, or a new input-bound request aliases a legacy/proposal identity | Empty/default input fields preserve old plan decoding. Input-bound requests use their own identity prefix and are Core-only until a separately versioned public request contract is admitted. |
