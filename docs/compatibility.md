...
| Windows launcher protocol V1 → V2 | Retained terminal Windows Jobs may carry `windows_native_launcher_v1`; new Runtime requires V2 runtime-context proof before new Windows admission | A new Runtime dispatches V2-only arguments through an old configured launcher, or rollback cannot decode V1 provider snapshots | V1 remains a decode-only historical provider contract. Candidate/deployment receipts move configured production to exact V2 launcher bytes and launcher path before the new Runtime accepts Windows work; release fencing prevents an old binary from inheriting nonterminal V2 Jobs. Remove V1 only after the rollback/persisted-state window proves no retained provider snapshot needs it. |
