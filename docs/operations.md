...
The default production release has one artifact authority rather than separate binary, operator-script, and supervisor-installation paths. It contains **15 receipt-bound artifacts**: five Rust binaries; the six installed Runtime operator executables (`deploy`, `lifecycle`, `reclaim`, `cache`, `status`, and `capacity-acceptance`); their shared `mcp_probe.py` support module; and the three repository-owned systemd units (`ordivon-runtime.service`, `ordivon-runtime-lifecycle.service`, and `ordivon-runtime-lifecycle.timer`). Every artifact is bound by name, kind, destination target, byte length, SHA-256 digest, and canonical mode. Executables use `0755`; `mcp_probe.py` and systemd units use `0644`. The only destination classes are `runtime_install` and `systemd_unit`: the former resolves below `--install-dir`, while the latter resolves below `--systemd-unit-dir` (default `/etc/systemd/system`). `plan` asks the live systemd manager for `UnitPath` and fails closed unless the selected unit root is actually searched by that manager, preventing a file-level “successful deployment” that would leave systemd loading older unit bytes. Passing explicit `--binary` values intentionally selects a binary-only subset for focused testing or exceptional maintenance; it is not the canonical production release set. On a source tree that contains the repository-owned Windows launcher and a usable Windows C# compiler, `prepare` also emits a separate `windowsProvider` record. That record binds `windows_native_launcher_v2`, an explicit build contract (`framework_csc_v1_nologo_optimize`), the launcher source digest, compiler path/digest, and the exact candidate PE digest; it remains separate from the 15 Linux/systemd release artifacts. The installed .NET Framework compiler is **not byte-deterministic**: repeated compilation of identical launcher source with the same compiler can vary the PE timestamp/MVID and therefore SHA-256. Runtime does not normalize or rewrite those PE bytes. Instead, release planning distinguishes the exact physical provider identity from its receipt-bound build provenance. If the currently configured exact launcher has a prior deployment receipt proving the same provider contract, build contract, source digest, and compiler digest as the new candidate, `transitionNeeded=false`: the current exact PE/path and environment assignment are preserved even when the freshly compiled candidate PE digest differs. The candidate remains exact fallback material. Missing or conflicting receipt provenance, a source change, compiler change, or build-contract change keeps the normal content-addressed provider transition. Historical V2 provider receipts that predate the explicit field map only to the pinned legacy `framework_csc_v1_nologo_optimize` contract; future build-recipe changes must use a new build-contract identity rather than reinterpret those receipts.

Eligibility requires:

- the exact requested Commit can be materialized from the source repository;
- `prepare` constructs a temporary detached Git checkout of that exact Commit and both builds binaries and stages operator/support/systemd-unit artifacts from that checkout, so mutable checkout state, including Git index flags such as `assume-unchanged`, cannot enter the candidate;
...
- the candidate manifest binds the exact source repository, Commit, `sourceMaterialization=detached_git_checkout`, candidate directory, complete release artifact set, destination targets, modes, sizes, and digests; for a default release it also proves that the selected `systemd_unit` root is present in the live manager `UnitPath`;
...
- when `ORDIVON_WINDOWS_LAUNCHER_PATH` and `ORDIVON_WINDOWS_WSL_DISTRIBUTION` configure a Windows provider, `plan` requires an exact candidate `windowsProvider`; it then either receipts a provenance-proven no-op that preserves the current exact provider, or installs the candidate bytes below the existing provider root at `sha256-<launcherDigest>/ordivon-windows-job-launcher.exe` and plans an exact one-line launcher-path update in the Runtime environment file;
...
The tool first stages `.next` files and receipt-local previous artifacts without replacing the running release. It then takes an exclusive Registry `admission.lock`. Runtime new admission takes a shared lock only after exact replay has been checked, so a deployed Runtime returns retryable `DEPLOYMENT_IN_PROGRESS` for new work while already committed requests remain replayable. Under the exclusive fence, `apply` waits for active/held reservations to drain naturally and then stops MCP ingress immediately. It rechecks the Registry with ingress closed, reruns the complete deployment plan, and writes that final plan to the receipt before any release replacement. Only then are installed `.previous` artifacts refreshed and the staged set atomically committed at their target-specific destinations. When systemd units are part of the release, their `.next` files are renamed into the manager-owned unit root under the same fence and `systemctl daemon-reload` runs **before** Runtime is restarted, so the subsequent service probe observes the exact receipt-bound unit bytes rather than a stale manager cache. On a Windows-configured node, the same fenced commit installs a new content-addressed launcher and atomically advances only `ORDIVON_WINDOWS_LAUNCHER_PATH` **when provider build provenance changed or cannot be proved equivalent**. If receipt-bound provenance proves the current provider equivalent, the release receipts an explicit provider no-op and leaves its exact PE/path and environment assignment unchanged. The receipt stores previous/new launcher paths plus environment-file before/after digests, not environment contents. If the post-commit probe fails, recovery restores the full previous artifact set including systemd units, reloads the manager when unit bytes were restored, and restores the prior launcher-path assignment. The new service must become `active`, complete modern discovery, expose the expected 22-Tool catalog including `release.apply`, `release.get`, `runtime.describe`, and `workspace.content`, and match the bound candidate identities.
